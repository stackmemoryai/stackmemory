...
export class LinearWebhookHandler {
   ...
   */
  private async handleIssueCreated(payload: LinearWebhookPayload): Promise<void> {
   ...
   */
  private async handleIssueUpdated(payload: LinearWebhookPayload): Promise<void> {
    ...
    let newStatus: 'pending' | 'in_progress' | 'completed' | 'cancelled' | undefined;
    
    if (payload.data.state) {
      const mappedStatus = this.mapLinearStateToStatus(payload.data.state) as 'pending' | 'in_progress' | 'completed' | 'cancelled';
   ...
   */
  private async handleIssueRemoved(payload: LinearWebhookPayload): Promise<void> {
   ...
   */
  private findMappingByLinearId(linearId: string): any {
   ...
   */
  private mapLinearStateToStatus(state: any): string {
