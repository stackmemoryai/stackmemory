...
Run these steps in order after every code change. Pre-commit hooks catch 80% of CI failures locally — catching failures here avoids broken builds on push:

1. `npm run lint` — fix ALL errors and warnings (warnings become errors in CI)
2. `npm run test:run` — confirm no regressions before proceeding
3. `npm run build` — verify compilation succeeds
4. Run the code directly to confirm it works end-to-end

Test coverage requirements:
- Every new feature needs tests in `src/**/__tests__/`
- Maintain or improve coverage — do not add code paths with no test coverage
- Critical paths requiring tests: context management, handoff, Linear sync

Testing rules (violations cause flaky or hanging tests):
- Run `npm run test:run` via subagent or background task — running it inline blocks the main context window
