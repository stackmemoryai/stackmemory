...
- **Do NOT import from `@jest/globals`** in `src/` tests — use global `jest`
- **Do NOT spawn subagents** for library doc lookups — use Context7 directly
- **Do NOT trigger `claude-api` skill** when touching `src/llm/` or `@anthropic-ai/sdk`
- **Do NOT run `npm test` inline** — it is long-running; use background process or sub-agent
- **Add `.js` extension** to all relative ESM imports
- **Use `catch {}`** not `catch (_err) {}` — underscore prefix not in allowed ESLint pattern
...
- Pre-commit hook runs: `npm run lint` + `npm run test` + E2E browser screenshots (opt-in with `PROVENANTAI_POST_COMMIT_E2E=1`)
...
### Running Tests
- `npm test` is long-running — always run in a background process or sub-agent
- Use `scripts/verify-staged.sh` for fast, targeted validation of changed files
...
- When context is injected via StackMemory MCP, treat it as authoritative background — do not re-fetch or re-derive what is already supplied